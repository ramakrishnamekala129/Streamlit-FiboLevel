...
if (status == 'Investing.com'):
	...
	if (select=='Nifty 50') or (select=='Nifty Bank'):
		...
		t=investpy.get_stock_historical_data(stock=select,
                                        country='India',
...
if currentfibo:
	...
	st.dataframe(df)
	st.text('Open Price for Calculation is '+str(float(todayprice/10000)))
if currentfiboweek:
	...
	print(dfw)
	st.dataframe(dfw)
	st.text('Week Open Price for Calculation is '+str(float(todaypricew)))
